...
public class KafkaConfig {
    ...
    @Bean
    public CounterEventRecordConsumer counterEventRecordConsumer() {
        ...
        KafkaConsumer<String, CounterEvent> kafkaConsumer = new KafkaConsumer<>(properties);

        kafkaConsumer.subscribe(Collections.singleton(applicationProperties.getTopic()));

        return new CounterEventRecordConsumer(kafkaConsumer, counterEventRecordStore());
    }

    @Bean
    public CounterEventRecordProducer counterEventRecordProducer() {
        Properties properties = new Properties();
        ...
        Producer<String, CounterEvent> kafkaProducer = new KafkaProducer<>(properties);

        return new CounterEventRecordProducer(kafkaProducer);
    }

    @Bean
    public CounterEventRecordStore counterEventRecordStore() {
        return new CounterEventRecordStore();
    }
}
